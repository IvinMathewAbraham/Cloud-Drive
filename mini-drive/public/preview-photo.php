...
$filePath = $basePath . $photo['storage_key'];
$servePath = $filePath;

if (!empty($photo['thumbnail_path'])) {
    $thumbPath = $basePath . $photo['thumbnail_path'];
...
if (!file_exists($servePath)) {
    ...
    exit('Preview not available');
}

$mime = Media::detectMime($servePath) ?: $photo['mime_type'] ?: 'image/jpeg';
header('Content-Type: ' . $mime);
header('Content-Disposition: inline; filename="' . basename($photo['original_filename']) . '"');
